...
public final class FrameGenerator {
    ...
    public void onWorldDepth() {
        ...
        if (irisShaderPackActive() || OptiFineBridge.shaderPackActive()) {
            if (!loggedIrisDepthOff) {
                loggedIrisDepthOff = true;
                Interframe.LOGGER.info("[Interframe] Shaderpack active (Iris or OptiFine) — translational "
                        + "reprojection disabled (the vanilla depth buffer isn't the shaderpack's); "
                        + "rotation warp still on.");
            }
            return;
        }
