...
public class InterframeConfig {
     ...
     * not just rotation. Costs one depth-buffer copy per frame; automatically off under Iris shaderpacks
     * (their depth isn't the vanilla buffer's) and wherever depth is unavailable.
     *
     * <p>Defaulted OFF: unlike the rotation warp (exact at any distance, scene-independent), this uses a
     * single depth sample per pixel from the CURRENT real frame only. Anything that isn't static
     * camera-locked geometry — swaying leaves/water, weather, mobs, other players, particles, the
     * player's own hand — moved independently of the camera between the two real frames, so warping it
     * by camera motion alone places it wrong. At depth discontinuities (block edges, mob silhouettes)
     * this shows up as a visible swimming/shaking artifact that gets worse the faster the camera moves —
     * i.e. exactly the "shakes when I move the camera" symptom. Rotation-only warp still delivers the
     * dominant smoothness benefit (mouse-look) without this artifact. Advanced users can re-enable this
     * from the options screen to trade that stability for sharper parallax during straight-line movement.
     */
    public boolean translationWarp = false;
