...
**AI frame generation for Minecraft** — Fabric 1.21.1 (ported from the original NeoForge mod). Captures
each finished frame and inserts synthesised *in-between* frames to raise the perceived frame rate and
smooth motion. Built for MC 1.21.1, Sodium 0.8, Iris, and Reese's Sodium Options, on Fabric Loader.
...
This port was written and reviewed without a live Fabric/Minecraft toolchain (no network access to fetch
Minecraft, mappings, Fabric Loader, or Sodium in the environment that produced it), so **it has not been
compiled or run**. Before relying on it:

```
./gradlew build
```

and fix anything Loom/the compiler flags — most likely candidates if something doesn't line up: the exact
Sodium 0.8 Fabric artifact/version string in `gradle.properties`, and the `sodium:config_api_user`
entrypoint signature (`ConfigEntryPoint`) if Sodium's Fabric API differs subtly from its NeoForge one.
Everything else (mixins, render-pipeline calls) uses Mojang's official mappings, identical to the
NeoForge original, so it should port over unchanged. Produces `build/libs/interframe-<version>.jar`;
requires JDK 21. Visual/latency tuning (pacing, look-ahead, warp strength) depends on your GPU/display and
needs in-game testing regardless of how cleanly it compiles.
